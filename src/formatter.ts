 ...
 *
 * @param recommendations - Array of recommendations from finfocus
 * @returns Total achievable savings (max per resource+action_type group)
 */
export function calculateAchievableSavings(recommendations: Recommendation[]): number {
...
`;
  ...
  const achievableSavings = recommendationsReport?.recommendations
    ? calculateAchievableSavings(recommendationsReport.recommendations)
    : undefined;
